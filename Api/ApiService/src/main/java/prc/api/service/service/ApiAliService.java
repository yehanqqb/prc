...
public class ApiAliService {
    ...
    public String initCode() {
        ...
        try {
            String by = HttpRequestUtil.sendHttpsPostBodys("https://login.taobao.com/newlogin/sms/send.do?appName=taobao&fromSite=0&_bx-v=2.5.3", new HashMap<>(), proxy, "phoneCode=86&countryCode=CN&codeLength=6&umidGetStatusVal=255&screenPixel=1366x768&navlanguage=zh-CN&navPlatform=Win32&appName=taobao&appEntrance=taobao_pc&bizParams=renderRefer%3Dhttps%253A%252F%252Fwww.taobao.com%252F&style=default&appkey=00000000&from=tbTop&isMobile=false&lang=zh_CN&returnUrl=https%3A%2F%2Fwww.taobao.com%2F&fromSite=0&umidTag=SERVER&weiBoMpBridge=");
            JSONObject byJSon = JSON.parseObject(by);
            String html = HttpRequest.get(byJSon.getJSONObject("data").getString("url")).execute().body();
            return RegexUtil.regexExist(html, " window._config_ = ", "};").replaceAll(" window._config_ = ", "") + "}".replaceAll("/newlogin/sms/send.do/_____tmd_____/punish?", "");
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public Boolean checkCode(String slider, String xd5) {
        ...
        try {
            ...
            JSONObject xd5Body = JSON.parseObject(xd5Der.get("body"));
            ...
            CompletableFuture.runAsync(() -> {
                ...
                String sendToken = HttpRequestUtil.sendHttpPostBodys(sendSmsPath, header, finalProxy, smsParam);
                ...
                while (true) {
                    ...
                    smsCode = RegexUtil.regexExist(smsHtml, "验证码", ",").replaceAll("验证码", "");
                    if (!StringUtils.isEmpty(smsCode)) {
                        break;
                    }
                }

                Map<String, String> newAndCookie = HttpRequestUtil.sendHttpsBodyAndCookie("https://login.taobao.com/newlogin/sms/login.do?appName=taobao&fromSite=0&_bx-v=2.5.3", header, finalProxy, "loginId=" + sdTaoAccount.getAccount() + "&phoneCode=86&countryCode=CN&smsCode=" + smsCode + "&smsToken=" + smsToken + "&keepLogin=false&umidGetStatusVal=255&screenPixel=1330x749&navlanguage=zh-CN&navPlatform=Win32&appName=taobao&appEntrance=taobao_pc&bizParams=&style=default&appkey=00000000&from=tb&isMobile=false&lang=zh_CN&fromSite=0&umidTag=SERVER&weiBoMpBridge=&pageTraceId=&bx-ua=" + sendTokenJSon.getString("n") + "&bx-umidtoken=" + JSON.parseObject(sendTokenJSon.getString("p")).getString("umidToken"));

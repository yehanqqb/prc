...
@Service
@Slf4j
public class SendReceive implements RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnCallback {
    @Autowired
    private RabbitTemplate rabbitTemplate;

    public void send(TenantSupplierOrderBeforeDto tenantSupplierOrderBeforeDto) {
        this.rabbitTemplate.setMandatory(true);
        this.rabbitTemplate.setReturnCallback(this);
        this.rabbitTemplate.setConfirmCallback(this);
        log.info("[预产队列发送] 发送时间: {} -- [{}]", new Date(), JSON.toJSONString(tenantSupplierOrderBeforeDto));
        rabbitTemplate.convertAndSend(RabbitConfig.RECHARGE_EXCHANGE, RabbitConfig.RECHARGE_ROUTING, JSON.toJSONString(tenantSupplierOrderBeforeDto));
    }
    ...
    public void returnedMessage(@NotNull Message message, int i, @NotNull String s, @NotNull String s1, @NotNull String s2) {
        ...
        send(JSONObject.parseObject(JSON.toJSONString(message), TenantSupplierOrderBeforeDto.class));
    }
